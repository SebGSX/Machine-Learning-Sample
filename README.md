...
The code is designed to run on Linux. The code is written using PyCharm on Windows, but is run using WSL 2.0 within
Windows using a clean Linux Ubuntu 24.04 LTS distribution.

> **Note:** TensorFlow typically lags behind by one minor version on Python and WSL only supports LTS Ubuntu releases.
> As such, check the TensorFlow documentation _before_ upgrading anything in this project.
...
> For the NVIDIA component installations, please refer to the official NVIDIA Developer documentation.

1. Install the NVIDIA CUDA Toolkit, version 13.2.
2. Ensure that Python 3.13, `pip`, and the `venv` module are installed.
3. Install `venv`. create the `machine_learning_sample` virtual environment, and activate it.
4. Install `pytorch` for CUDA 13.2, `tensorflow`, `cupy-cuda12x`, `scipy`, `optuna`, `sympy`, `matplotlib`, `pandas`,
   `kagglehub`, `coverage`, `pytest`, and `pytest-mock`.

In Ubuntu 24.04, Python 3.12 is installed by default and must be upgraded to Python 3.13. The following commands are
used to install the Python 3.13 and the necessary components:
...
```shell
python3.13 -m venv ~/.local/share/virtualenvs/machine_learning_sample
...
pip3 install cupy-cuda13x scipy optuna sympy matplotlib pandas kagglehub coverage pytest pytest-mock
```
...
To create a `requirements.txt` file, the following command is run within the project's root directory:
